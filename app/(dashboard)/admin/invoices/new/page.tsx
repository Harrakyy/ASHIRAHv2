  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (customersError) {
          console.error('Customers error:', customersError.code, customersError.message, customersError.hint)
        ...
        if (servicesError) {
          console.error('Services error:', servicesError.code, servicesError.message, servicesError.hint)
          throw servicesError
        }
        ...
        if (ordersError) {
          console.error('Orders error:', ordersError.code, ordersError.message, ordersError.hint)
          throw ordersError
        }
...
const handlePrint = () => {
  ...
  printWindow.document.write(`
    ...
    <html>
      <head>
        <title>Invoice INV-{currentYear}-001</title>
        <style>
  ...
  return (
    <div className="p-4 md:p-6 overflow-hidden bg-gray-50 dark:bg-gray-950 min-h-screen">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Buat Invoice Baru</h1>
          <p className="text-gray-500 dark:text-gray-400">No. Invoice: INV-{currentYear}-001</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 overflow-hidden">
        ...
        <div className="space-y-4 md:space-y-6">
          <Card className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
            <CardHeader>
              <CardTitle className="text-lg text-gray-900 dark:text-white">Detail Invoice</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <Label className="text-gray-700 dark:text-gray-300">Pelanggan</Label>
                <Select value={selectedCustomer} onValueChange={(v) => { setSelectedCustomer(v); setSelectedOrder(""); }}>
                  ...
                  <SelectContent className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-lg dark:shadow-gray-900">
                    {customers.length === 0 ? (
                      ...
                      customers.map((customer) => (
                        <SelectItem key={customer.id} value={customer.id} className="text-gray-900 dark:text-white focus:bg-gray-100 dark:focus:bg-gray-700">
                          <div>
                            <p className="text-gray-900 dark:text-white">{customer.full_name || "Tanpa Nama"}</p>
              ...
              <div>
                <Label className="text-gray-700 dark:text-gray-300">Pesanan (Opsional)</Label>
              ...
              <div>
                <Label className="text-gray-700 dark:text-gray-300">Atau Layanan (Manual)</Label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-gray-700 dark:text-gray-300">Jatuh Tempo</Label>
                  <Input
                    type="date"
                    value={dueDate}
                ...
                <div>
                  <Label className="text-gray-700 dark:text-gray-300">Pajak (%)</Label>
                  <Input
                    type="number"
                    value={taxRate}
          ...
          <Card className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 rounded-xl">
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle className="text-lg text-gray-900 dark:text-white">Item</CardTitle>
            ...
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow className="border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                    <TableHead className="w-[40%] text-gray-500 dark:text-gray-400">Deskripsi</TableHead>
                    <TableHead className="text-gray-500 dark:text-gray-400">Qty</TableHead>
                    <TableHead className="text-gray-500 dark:text-gray-400">Harga</TableHead>
                    <TableHead className="text-gray-500 dark:text-gray-400">Subtotal</TableHead>
                    <TableHead className="text-gray-500 dark:text-gray-400"></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {lineItems.map((item) => (
                    <TableRow key={item.id} className="border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white">
                      <TableCell>
                        <Input
                          ...
                          className="bg-transparent dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
                        />
                      </TableCell>
                      <TableCell>
                        <Input
                          ...
                          className="w-20 bg-transparent dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white"
                        />
                      </TableCell>
                      <TableCell>
                        <Input
                          ...
                          className="w-32 bg-transparent dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white"
                        />
                      </TableCell>
                      ...
                      <TableCell>
                        ...
                        >
                          <Trash2 className="h-4 w-4 text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300" />
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))}
              ...
              <div className="mt-4 space-y-2 text-right bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                <div className="flex justify-end gap-8">
                  <span className="text-gray-600 dark:text-gray-400">Subtotal</span>
                  <span className="font-medium w-32 text-gray-900 dark:text-white">{formatCurrency(subtotal)}</span>
                </div>
                <div className="flex justify-end gap-8">
                  <span className="text-gray-600 dark:text-gray-400">Pajak ({taxRate}%)</span>
                  <span className="font-medium w-32 text-gray-900 dark:text-white">{formatCurrency(taxAmount)}</span>
                </div>
                <div className="flex justify-end gap-8 text-lg pt-2 border-t border-gray-200 dark:border-gray-700">
                  <span className="font-bold text-gray-900 dark:text-white">Grand Total</span>
                  <span className="font-bold w-32 text-gray-900 dark:text-white">{formatCurrency(grandTotal)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 rounded-xl">
            <CardHeader>
              <CardTitle className="text-lg text-gray-900 dark:text-white">Catatan / Syarat Pembayaran</CardTitle>
            </CardHeader>
            <CardContent>
              <Textarea
                value={notes}
          ...
          <div className="flex flex-col sm:flex-row gap-3 overflow-hidden">
            <Button variant="outline" className="w-full sm:flex-1 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 overflow-hidden">
        ...
        {/* Preview */}
        <Card className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 rounded-xl sticky top-6 print-container">
          <CardHeader className="print-hidden">
            <CardTitle className="text-lg text-gray-900 dark:text-white">Preview Invoice</CardTitle>
          </CardHeader>
          <CardContent className="print-invoice-content">
            <div id="invoice-card" className="border border-gray-200 dark:border-gray-700 rounded-lg p-6 bg-white dark:bg-gray-900 print:border-0 print:p-0">
              {/* Invoice Header */}
              <div className="flex justify-between items-start mb-8">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">INV-{currentYear}-001</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-8 mb-8">
                ...
                <div className="text-right">
                  ...
                  <p className="font-medium text-gray-900 dark:text-white">{dueDate || "-"}</p>
                </div>
              </div>
              ...
              <table className="w-full mb-8">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                    <th className="text-left py-2 text-sm text-gray-500 dark:text-gray-400">Deskripsi</th>
                    <th className="text-right py-2 text-sm text-gray-500 dark:text-gray-400">Qty</th>
                    <th className="text-right py-2 text-sm text-gray-500 dark:text-gray-400">Harga</th>
                    <th className="text-right py-2 text-sm text-gray-500 dark:text-gray-400">Subtotal</th>
                  </tr>
                </thead>
                <tbody>
                  {lineItems.map((item) => (
                    <tr key={item.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                      ...
                      <td className="py-2 text-sm text-right text-gray-900 dark:text-white">{item.qty}</td>
              ...
              <div className="space-y-2 text-right mb-8 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                <div className="flex justify-end gap-8">
                  ...
                  <span className="text-sm w-28 text-gray-900 dark:text-white">{formatCurrency(subtotal)}</span>
                </div>
                <div className="flex justify-end gap-8">
                  <span className="text-sm text-gray-600 dark:text-gray-400">
                    Pajak ({taxRate}%)
                  </span>
                  <span className="text-sm w-28 text-gray-900 dark:text-white">{formatCurrency(taxAmount)}</span>
                </div>
                <div className="flex justify-end gap-8 pt-2 border-t border-gray-200 dark:border-gray-700">
                  <span className="font-bold text-gray-900 dark:text-white">Grand Total</span>
                  <span className="font-bold w-28 text-gray-900 dark:text-white">{formatCurrency(grandTotal)}</span>
                </div>
              </div>
              ...
              {notes && (
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <p className="text-sm text-gray-900 dark:text-white whitespace-pre-line">{notes}</p>
                </div>
              )}
            </div>
          </CardContent>
