...
export function HoldingCards() {
  return (
    <section id="holdings" className="py-24 lg:py-32 bg-navy">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-white leading-tight mb-6 text-balance">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {holdings.map((holding, index) => (
            <Link
              key={index}
              href={holding.href}
              className={`group relative overflow-hidden border transition-all duration-500 hover:scale-[1.02] rounded-xl ${
                holding.theme === 'dark' 
            ...
            >
              <div className="p-8 lg:p-10">
                {/* Logo */}
                <div className={`mb-6 flex items-center ${holding.theme === 'dark' ? 'h-24' : 'h-16 grayscale group-hover:grayscale-0 transition-all'}`}>
                  <Image
                    src={holding.logo}
                    alt={holding.name}
                    width={holding.theme === 'dark' ? 340 : 160}
                    height={holding.theme === 'dark' ? 136 : 64}
                    className="object-contain object-left"
                    style={{ width: 'auto', height: 'auto', maxHeight: holding.theme === 'dark' ? '136px' : '64px' }}
                  />
                </div>
